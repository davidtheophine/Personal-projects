...
def buy():
    """Buy shares of stock"""
    if request.method == "POST":
        missing = info_entered("symbol") or info_entered("shares")
        if missing:
            return missing
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("input must be symbol")
        ...
        db.execute("UPDATE user SET cash = cash - :total_cost WHERE id = user_id",
                   total_cost=session["user_id"],symbol=symbol, shares=shares, price=price)
